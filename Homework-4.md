...
3. Load that repo and its corresponding issues from the Redis database that you created in HW1, using the Part A persistence framework. 
4. Clone the repository (feel free to use the `git` binary, via the `ProcessBuilder`, or any other library).
5. For the issues in that repo, invoke Microservice A to summarize them. Let's call this `IssueList1`.
6. Load the `.c` files, specified in `selected_repo.dat`, in that repo and invoke Microservice B to get the list of bugs in it. Let's call this `IssueList2`.
7. Send both `IssueList1` and `IssueList2` to Microservice C and print the set of issues that it says are common - that the LLM identified it from the source code AND it was mentioned in a GitHub Issue.
8. In `README.md` discuss the results.
